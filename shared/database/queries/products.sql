...
WHERE ((sqlc.narg(name)::varchar IS NULL OR "name" ILIKE '%' || sqlc.narg(name)::varchar || '%') OR (sqlc.narg(name)::varchar IS NULL OR name_en ILIKE '%' || sqlc.narg(name)::varchar || '%'))
  ...
  AND (
    sqlc.narg(category_id)::bigint[] IS NULL
        ...
        OR categories = ANY (sqlc.narg(category_id)::bigint[])
    )
ORDER BY CASE
             WHEN sqlc.arg(order_by_type)::text = 'asc' THEN
                 CASE
                     WHEN sqlc.arg(order_by)::text = 'id' THEN id::text
                     WHEN sqlc.arg(order_by)::text = 'name' THEN "name"
                     WHEN sqlc.arg(order_by)::text = 'price' THEN price::text
                     ELSE id::text
                     END
             END,
         CASE
